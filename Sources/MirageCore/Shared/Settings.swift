...
private enum SettingLogger {
    static let logger = PlatformLogger(subsystem: "Setting<T>", category: "Mirage")
}
    ...
    private func getScalarValue(_ type: T.Type, forKey key: String) -> T? {
        if let data = UserDefaults.standard.data(forKey: key) {
            do {
                ...
                SettingLogger.logger.error(error, task: "Get Setting Value of type \(T.self) for '\(key)'")
                return defaultValue
            }
        } else {
            return defaultValue
    ...
    private func setScalarValue(_ value: T?, forKey key: String) {
        if let value {
            do {
                ...
                SettingLogger.logger.error(error, task: "Set Setting Value of type \(T.self) for '\(key)'")
            }
        } else {
            UserDefaults.standard.removeObject(forKey: key)
        }
