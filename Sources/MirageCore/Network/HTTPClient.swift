...
extension HTTPClient {
    ...
    fileprivate struct ClientRequest {
        ...
        init(url: URL, method: Method, payload: Payload, accept: ContentType, logOptions: LogOptions? = nil, headers: [String: String]? = nil, timeout: TimeInterval? = nil) {
...
extension HTTPClient {
    ...
    fileprivate struct ClientResponse {
        ...
        private static func extractHeaders(from httpURLResponse: HTTPURLResponse) -> (StatusCode?, [String: String]?) {
            ...
            return (StatusCode(rawValue: httpURLResponse.statusCode), headers.isEmpty ? nil : headers)
        }
...
public extension HTTPClient {
    ...
    func get(_ url: URL, headers: [String: String]? = nil) async throws -> Data? {
        let (data, _) = try await request(url, method: .get, headers: headers)
        return data
    }

    func get(_ urlRequest: URLRequest) async throws -> Data? {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        let (data, _) = try await request(url, method: .get, headers: urlRequest.allHTTPHeaderFields ?? [:])
        return data
    }

    func get<Output: Decodable>(_ url: URL, decoding outputType: Output.Type, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        try await request(url, method: .get, data: nil, outputType: Output.self, headers: headers, userInfo: userInfo)
    }

    func get<Output: Decodable>(_ urlRequest: URLRequest, decoding outputType: Output.Type, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .get, data: nil, outputType: Output.self, headers: urlRequest.allHTTPHeaderFields, userInfo: userInfo)
    }
}
...
public extension HTTPClient {
    ...
    func post(_ url: URL, headers: [String: String]? = nil) async throws -> Data? {
        try await request(url, method: .post, headers: headers).0
    }

    func post(_ urlRequest: URLRequest) async throws -> Data? {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .post, headers: urlRequest.allHTTPHeaderFields ?? [:]).0
    }

    func post<Output: Decodable>(_ url: URL, data: Data, decoding outputType: Output.Type, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        try await request(url, method: .post, data: data, outputType: outputType, headers: headers, userInfo: userInfo)
    }

    func post<Output: Decodable>(_ urlRequest: URLRequest, data: Data, decoding outputType: Output.Type, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .post, data: data, outputType: outputType, headers: urlRequest.allHTTPHeaderFields ?? [:], userInfo: userInfo)
    }

    func post(_ url: URL, payload input: some Encodable, headers: [String: String]? = nil) async throws -> Data? {
        try await request(url, method: .post, input: input, headers: headers).0
    }

    func post(_ urlRequest: URLRequest, payload input: some Encodable) async throws -> Data? {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .post, input: input, headers: urlRequest.allHTTPHeaderFields ?? [:]).0
    }

    func post<Output: Decodable>(_ url: URL, payload input: some Encodable, decoding outputType: Output.Type, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        try await request(url, method: .post, input: input, outputType: outputType, headers: headers, userInfo: userInfo)
    }

    func post<Output: Decodable>(_ urlRequest: URLRequest, payload input: some Encodable, decoding outputType: Output.Type, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .post, input: input, outputType: outputType, headers: urlRequest.allHTTPHeaderFields, userInfo: userInfo)
    }
    ...
    func post(_ url: URL, form: MultipartForm, headers: [String: String]? = nil) async throws -> Data? {
    ...
    func post(_ urlRequest: URLRequest, form: MultipartForm) async throws -> Data? {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await post(url, form: form, headers: urlRequest.allHTTPHeaderFields)
    }

    func post<Output: Decodable>(_ url: URL, form: MultipartForm, decoding outputType: Output.Type, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        let payload = payload(for: form)
    ...
    func post(_ url: URL, form: URLEncodedForm, headers: [String: String]? = nil) async throws -> Data? {
        let payload = payload(for: form)
    ...
    func post(_ urlRequest: URLRequest, form: URLEncodedForm) async throws -> Data? {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await post(url, form: form, headers: urlRequest.allHTTPHeaderFields)
    }

    func post<Output: Decodable>(_ url: URL, form: URLEncodedForm, decoding outputType: Output.Type, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        let payload = payload(for: form)
    ...
    func post<Output: Decodable>(_ urlRequest: URLRequest, form: URLEncodedForm, decoding outputType: Output.Type, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await post(url, form: form, decoding: outputType, headers: urlRequest.allHTTPHeaderFields, userInfo: userInfo)
    }
}
...
public extension HTTPClient {
    ...
    func put(_ url: URL, headers: [String: String]? = nil) async throws -> Data? {
        try await request(url, method: .put, headers: headers).0
    }

    func put(_ urlRequest: URLRequest) async throws -> Data? {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .put, headers: urlRequest.allHTTPHeaderFields).0
    }

    func put<Output: Decodable>(_ url: URL, data: Data, decoding outputType: Output.Type, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        try await request(url, method: .put, data: data, outputType: outputType, headers: headers, userInfo: userInfo)
    }

    func put<Output: Decodable>(_ urlRequest: URLRequest, data: Data, decoding outputType: Output.Type, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .put, data: data, outputType: outputType, headers: urlRequest.allHTTPHeaderFields, userInfo: userInfo)
    }

    func put(_ url: URL, payload input: some Encodable, headers: [String: String]? = nil) async throws -> Data? {
        try await request(url, method: .put, input: input, headers: headers).0
    }

    func put(_ urlRequest: URLRequest, payload input: some Encodable) async throws -> Data? {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .put, input: input, headers: urlRequest.allHTTPHeaderFields).0
    }

    func put<Output: Decodable>(_ url: URL, payload input: some Encodable, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        try await request(url, method: .put, input: input, outputType: Output.self, headers: headers, userInfo: userInfo)
    }

    func put<Output: Decodable>(_ urlRequest: URLRequest, payload input: some Encodable, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .put, input: input, outputType: Output.self, headers: urlRequest.allHTTPHeaderFields, userInfo: userInfo)
    }
}
...
public extension HTTPClient {
    ...
    func delete(_ url: URL, headers: [String: String]? = nil) async throws -> Data? {
        try await request(url, method: .delete, headers: headers).0
    }

    func delete(_ urlRequest: URLRequest) async throws -> Data? {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .delete, headers: urlRequest.allHTTPHeaderFields).0
    }

    func delete<Output: Decodable>(_ url: URL, data: Data, decoding outputType: Output.Type, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        try await request(url, method: .delete, data: data, outputType: outputType, headers: headers, userInfo: userInfo)
    }

    func delete<Output: Decodable>(_ urlRequest: URLRequest, data: Data, decoding outputType: Output.Type, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .delete, data: data, outputType: outputType, headers: urlRequest.allHTTPHeaderFields, userInfo: userInfo)
    }

    func delete(_ url: URL, payload input: some Encodable, headers: [String: String]? = nil) async throws -> Data? {
        try await request(url, method: .delete, input: input, headers: headers).0
    }

    func delete(_ urlRequest: URLRequest, payload input: some Encodable) async throws -> Data? {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .delete, input: input, headers: urlRequest.allHTTPHeaderFields).0
    }
    ...
    func delete<Output: Decodable>(_ urlRequest: URLRequest, payload input: some Encodable, decoding outputType: Output.Type, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        guard let url = urlRequest.url else { throw Error.missingURL(context: "\(#file):\(#function)") }
        return try await request(url, method: .delete, input: input, outputType: Output.self, headers: urlRequest.allHTTPHeaderFields, userInfo: userInfo)
    }
}
...
extension HTTPClient {
    ...
    private func request(_ url: URL, method: Method, headers: [String: String]? = nil) async throws -> (
    ...
    private func request<Output: Decodable>(_ url: URL, method: Method, data: Data?, outputType: Output.Type, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]? = nil) async throws -> Output {
        let payload =
            if let data {
                Payload(
                    data: data,
    ...
    private func request<Input: Encodable>(_ url: URL, method: Method, input: Input, headers: [String: String]? = nil) async throws -> (Data?, HTTPURLResponse) {
    ...
    private func request<Input: Encodable, Output: Decodable>(_ url: URL, method: Method, input: Input, outputType: Output.Type, headers: [String: String]? = nil, userInfo: [CodingUserInfoKey: Sendable]?) async throws -> Output {
