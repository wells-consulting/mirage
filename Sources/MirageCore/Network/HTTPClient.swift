...
public actor HTTPClient {
    ...
    ) async throws(MirageError) -> (Data?, HTTPURLResponse) {
...
extension HTTPClient {
    ...
    struct ClientRequest {
        ...
        init(
            ...
            self.init(refcode: refcode, urlRequest: urlRequest, payload: payload, logOptions: logOptions)
        }
    }
    ...
    struct Payload: SummaryProviding, Sendable {
...
public extension HTTPClient {
    ...
    func get(
        ...
        headers: [String: String]? = nil,
        refcode: String? = nil,
    ) async throws(MirageError) -> Data? {

        let (data, _) = try await request(
            refcode: refcode ?? "JVD6",
            url: url,
            method: .get,
            headers: headers)
    ...
    func get(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "DX3D")
        }

        let (data, _) = try await request(
            refcode: refcode ?? "FNKW",
            url: url,
            method: .get,
    ...
    func get<Output: Decodable>(
        ...
        refcode: String? = nil,
    ) async throws(MirageError) -> Output {

        try await request(
            refcode: refcode ?? "SERE",
            url: url,
            method: .get,
            data: nil,
    ...
    func get<Output: Decodable>(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "G9S2")
        }

        return try await request(
            refcode: refcode ?? "BQUU",
            url: url,
            method: .get,
            data: nil,
            outputType: Output.self,
...
public extension HTTPClient {
    ...
    func post(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "PEME")
        }

        return try await request(
            refcode: refcode ?? "SJTU",
            url: url,
            method: .post,
            headers: urlRequest.allHTTPHeaderFields ?? [:]
        ).0
    ...
    func post<Output: Decodable>(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "J36P")
        }

        return try await request(
            refcode: refcode ?? "DUEJ",
            url: url,
            method: .post,
            data: data,
            outputType: outputType,
    ...
    func post(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "EFDU")
        }

        return try await request(
            refcode: refcode ?? "W84M",
            url: url,
            method: .post,
            input: input,
            headers: urlRequest.allHTTPHeaderFields ?? [:]
    ...
    func post<Output: Decodable>(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "59YK")
        }

        return try await request(
            refcode: refcode ?? "72HQ",
            url: url,
            method: .post,
            input: input,
            outputType: outputType,
    ...
    func post(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "GCRM")
        }
    ...
    func post(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "CJNY")
        }
    ...
    func post<Output: Decodable>(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "235J")
        }
...
public extension HTTPClient {
    ...
    func put(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "N2SR")
        }

        return try await request(
            refcode: refcode ?? "G5JY",
            url: url,
            method: .put,
            headers: urlRequest.allHTTPHeaderFields
        ).0
    ...
    func put<Output: Decodable>(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "5MNP")
        }

        return try await request(
            refcode: refcode ?? "5R82",
            url: url,
            method: .put,
            data: data,
            outputType: outputType,
    ...
    func put(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "JPHN")
        }

        return try await request(
            refcode: refcode ?? "R52F",
            url: url,
            method: .put,
            input: input,
            headers: urlRequest.allHTTPHeaderFields
    ...
    func put<Output: Decodable>(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "VB4M")
        }

        return try await request(
            refcode: refcode ?? "DUBG",
            url: url,
            method: .put,
            input: input,
            outputType: Output.self,
...
public extension HTTPClient {
    ...
    func delete(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "RKGT")
        }

        return try await request(
            refcode: refcode ?? "33FS",
            url: url,
            method: .delete,
            headers: urlRequest.allHTTPHeaderFields
        ).0
    ...
    func delete<Output: Decodable>(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "J8CV")
        }

        return try await request(
            refcode: refcode ?? "EY76",
            url: url,
            method: .delete,
            data: data,
            outputType: outputType,
    ...
    func delete(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "SWHT")
        }

        return try await request(
            refcode: refcode ?? "2NSM",
            url: url,
            method: .delete,
            input: input,
            headers: urlRequest.allHTTPHeaderFields
    ...
    func delete<Output: Decodable>(
        ...
        guard let url = urlRequest.url else {
            throw HTTPError.missingURL(refcode: refcode ?? "TSRS")
        }

        return try await request(
            refcode: refcode ?? "PJ38",
            url: url,
            method: .delete,
            input: input,
            outputType: Output.self,
