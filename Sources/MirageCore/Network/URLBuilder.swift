...
public final class URLBuilder {
    ...
    public convenience init(
        ...
        try self.init(url.absoluteString, dateFormatter: dateFormatter, refcode: refcode)
    }
    ...
    ///
    public func build(refcode: String? = nil) throws(URLError) -> URL {
        var components = URLComponents()

        guard let scheme else {
            throw .urlMissingScheme(
                refcode: refcode ?? "Q4DF",
...
public extension URL {
    ...
    static func from(_ string: String, refcode: String? = nil) throws(URLError) -> URL {
        guard let url = URL(string: string) else {
            throw .urlInvalid(
                refcode: refcode ?? "ECHA",
                urlString: string,
                urlComponents: nil)
        }
