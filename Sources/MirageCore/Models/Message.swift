...
public struct Message: Hashable, Codable, Sendable & SummaryProviding & DetailsProviding & TitleProviding {
    ...
    public static func error(_ error: Error, details: String? = nil, title: String? = nil) -> Self {
        ...
        let details = details ?? error.localizedDescription
        let title = title ?? ((error as? TitleProviding)?.title)
        return .init(summary: summary, details: details, title: title, severity: .error)
    }

    /// Message severity
    public enum Severity: Int, Equatable, Comparable, Codable, Sendable, TitleProviding {
        case info
        case warning
        case error
