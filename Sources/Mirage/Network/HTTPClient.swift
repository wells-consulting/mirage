...
public actor HTTPClient {
    ...
    private func sendRequest(_ clientRequest: ClientRequest) async throws -> (Data?, HTTPURLResponse) {
        ...
        guard let httpURLResponse = urlResponse as? HTTPURLResponse else {
            throw Self.Error(clientRequest: clientRequest, httpURLResponse: nil, data: data)
        }

        // Unrecoverable error: status code must be known

        guard let statusCode = StatusCode(rawValue: httpURLResponse.statusCode) else {
            throw Self.Error(clientRequest: clientRequest, httpURLResponse: httpURLResponse, data: data)
        }
...
public extension HTTPClient {
    ...
    struct Error: MirageError {
        ...
        fileprivate init(_ description: String, title: String? = nil, urlRequest: URLRequest? = nil, httpURLResponse: HTTPURLResponse? = nil, timeRange: Range<Date>? = nil, underlyingError: (any Swift.Error)? = nil, context: [String: any Sendable]? = nil) {
            self.title = title
        ...
        fileprivate init(clientRequest: ClientRequest, httpURLResponse: HTTPURLResponse?, data: Data? = nil, title: String? = nil, timeRange: Range<Date>? = nil, underlyingError: (any Swift.Error)? = nil, context: [String: any Sendable]? = nil) {
            ...
            if let methodRawValue = clientRequest.urlRequest.httpMethod, let method = Method(rawValue: methodRawValue) {
                if let statusCodeRawValue = httpURLResponse?.statusCode, let statusCode = StatusCode(rawValue: statusCodeRawValue) {
